...
=item Pipeline Download Link: xxxxxx.tar.gz

=item Additional Demo Data: xxxxxx.tar.gz

=back

After downloading the pipeline, unzip the files as follows. 

 your-folder$ tar -xzvf xxxxxx.tar.gz

It is runnable once you have R and Statistics::R on your machine. See below for the requirements. 

Similarly for the additional demo data, you can unzip them in the data file in C<your-folder/asarp_ss/data>

 ~$ mv xxxx.tar.gz your-folder/asarp_ss/data
 ~$ cd your-folder/asarp_ss/data
 ~your-folder/asarp_ss/data$ tar -xzvf xxxx.tar.gz

See the details in Quick Start: Demo
...
Second, users need to install the Perl package Statistics::R which the pipeline uses to interact with R. Installation of Perl packages can be found in detail at CPAN: http://www.cpan.org/modules/INSTALL.html. The documentation is detailed and it is relatively easy for sudo users. 
...
For B<non-root users without sudo privilege>, one option is first install CPANM in a local directory, and install Perl packages locally using: C<cpanm Statistics::R>. See below for details.

=head1
Easy installation of Perl modules (as a non-root user)

Credits from (the original website is unfortunately gone; a slightly modified copy is provided below): www.tagestad.no/2010/10/easy-installation-of-perl-modules.html

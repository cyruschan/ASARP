...
The pipeline is maintained using GIT (http://git-scm.com/ ). To download the pipeline, simply go to the GitHub (https://github.com/ ) repository (or any other GIT repository) C<asarp-repository>, and C<clone> the project to C<your-folder>

 git clone asarp-repository your-folder

As it is available on GitHub, you can get it using:
